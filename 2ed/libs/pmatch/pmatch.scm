...
; e.g. plt-r5rs has no error and printf, so we must use r5rs.scm, see eopl: prefix.
...
(define-syntax pmatch-aux
  (syntax-rules (else guard)
    ...
    ((_ name v)
     (begin
       (if 'name
           (eopl:printf "pmatch ~s failed\n~s\n" 'name v)
           (eopl:printf "pmatch failed\n ~s\n" v))
       (eopl:error 'pmatch "match failed")))
